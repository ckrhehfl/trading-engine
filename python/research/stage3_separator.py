...
class SeparatorTest:
    ...
    mean_low: float
    null_sd: float
    #: Pooled standard deviation of the individual event forward returns,
    #: so `event_sd * sqrt(1/n_high + 1/n_low)` is the difference's own
    #: analytic standard error. Carried so `null_calibration` can be
    #: reported rather than assumed -- see `rd-l` §4.1 for the stage-2
    #: fault that made this mandatory.
    event_sd: float
    p_value: float
    permutations: int
    tvd_decile: float
    ...
    @property
    def analytic_se(self) -> float:
        """`event_sd * sqrt(1/n_high + 1/n_low)` — what the difference's
        standard error must be if the null is calibrated to it."""
        if self.n_high <= 0 or self.n_low <= 0:
            return float("nan")
        return self.event_sd * math.sqrt(1.0 / self.n_high + 1.0 / self.n_low)

    @property
    def null_calibration(self) -> float:
        """`null_sd / analytic_se`. **1.0 means the null is calibrated to
        the statistic it judges.**

        Reported because rd-l §4.1 found stage 2's matched null running as
        low as **0.42** — it matched on *prior* volatility while the events
        were *forward* volatility bursts, so its reference distribution was
        narrower than the statistic and every permutation p was too small.
        That went unnoticed through three nulls and two result documents
        for want of exactly this ratio.

        A label permutation should come out at 1.0 **by construction**:
        both branches are drawn from the same events, so the permutation
        distribution *is* the sampling distribution of the difference. That
        is rd-m §2's central claim, and printing this number is what turns
        it from an argument into a measurement.
        """
        se = self.analytic_se
        return self.null_sd / se if se > 0 else float("nan")
...
def report(results: list[SeparatorTest]) -> None:
    ...
    print(
        ...
        f"{'hi':>8} {'lo':>8} {'diff':>8} {'sd':>7} {'cal':>5} {'p':>9} {'rank':>5} "
    ...
    for r in sorted(results, key=lambda x: x.p_value):
        print(
            ...
            f"{r.difference*1e4:>8.2f} {r.null_sd*1e4:>7.2f} "
            f"{r.null_calibration:>5.2f} {r.p_value:>9.2e} "
    ...
    if results:
        cals = [r.null_calibration for r in results]
        print(
            f"null calibration (null_sd / analytic se): min {min(cals):.3f}, "
            f"max {max(cals):.3f}.\n  1.00 is a null calibrated to the statistic it "
            f"judges. A label permutation should reach it by\n  construction -- both "
            f"branches come from the same events -- and rd-l §4.1 found stage 2's\n  "
            f"matched null at 0.42, unnoticed through three nulls for want of this "
            f"ratio."
        )
    print(
