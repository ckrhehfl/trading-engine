"""Tests for `python/research/strategies/macro_sp500_trend.py` --
Strategy Research Task Y (the macro-sp500-trend strategy, the direct
structural sibling of Task X's `macro_real_yield_trend.py`). See
CLAUDE.md's "Strategy Research Methodology" section and
`.planning/sr-y-macro-sp500-strategy.md` for the full design context.
...
every `Kline`/`ObservationRow` here is hand-built. Matches
`test_macro_real_yield_trend.py`'s own established convention.
"""
...
from research.strategies.macro_sp500_trend import (
    DEFAULT_LOOKBACK_TRADING_DAYS,
    DEFAULT_REFERENCE_EQUITY,
    MacroSp500TrendStrategy,
    MacroSp500TrendTrainable,
    _sign,
    compute_sp500_trend_signs,
)
...
class TestComputeSp500TrendSigns:
    ...
    def test_output_preserves_the_later_dates_own_observation_date(self):
        ...
        result = compute_sp500_trend_signs(observations, lookback=1)
    ...
    def test_fewer_real_observations_than_lookback_yields_empty_result(self):
        ...
        result = compute_sp500_trend_signs(observations, lookback=5)
        assert result == []
...
class TestNonInversionAndDirection:
    """The single most important test class in this file: the whole point
    of this strategy versus its `macro_real_yield_trend` sibling is that
    the sign mapping is NOT inverted -- a copy-paste of the sibling's
    inversion logic would silently encode the WRONG hypothesis while
    still 'running' without error. Both directions are pinned explicitly,
    exactly mirroring `test_macro_real_yield_trend.py`'s own
    `TestInversionAndDirection` class in structure, but with the
    assertions swapped: here a RISING trend must produce LONG (not
    SHORT), and a FALLING trend must produce SHORT (not LONG).
    ...
    trend-driven opening it actually asserts on.
    """

    def test_a_rising_sp500_trend_produces_a_long_signal(self):
        # trend_observations carries a POSITIVE (rising-S&P500) sign dated
        # at/before the kline -- the strategy must go LONG (the
        # NON-inverted, same-direction mapping this module's docstring
        # specifies -- risk-on co-movement, not the sibling's inverse
        # relationship).
    ...
    def test_a_falling_sp500_trend_produces_a_short_signal(self):
        trend_observations = [_obs(_date_str(0), "-1")]
        strategy = _strategy(trend_observations=trend_observations)
        klines = _klines(3)
        intents = [strategy(klines[: i + 1]) for i in range(len(klines))]
    ...
    def test_flip_from_short_to_long_when_the_underlying_trend_reverses(self):
        # Day 0-2: trend = -1 (forward-filled), vol warms by index 2 ->
        # opens SHORT at index 2 (non-inverted: falling S&P500 -> short).
        # Day 3: trend flips to +1 -> flips LONG.
...
class TestOrderEmissionSignChangeOnly:
    ...
    def test_flatten_when_trend_nets_to_exactly_zero(self):
        # Opens LONG once vol warms (index 2, rising trend); trend nets to
        # zero on day 3.
...
class TestFitNoSearch:
    def _trainable(self, tmp_path, **overrides: Any) -> MacroSp500TrendTrainable:
        kwargs: dict[str, Any] = dict(
            strategy_id="test-macro-sp500-trend",
            strategy_version="v1",
            fee_bps=Decimal("0"),
            ...
            # non-empty trend-sign series -- mirrors
            # test_macro_real_yield_trend.py's identical fixture-sizing
            # reasoning (a single-observation fixture would make
            # compute_sp500_trend_signs return [] for the default
            # lookback=63).
        ...
        kwargs.update(overrides)
        return MacroSp500TrendTrainable(**kwargs)
    ...
    def test_fit_returns_fresh_strategy_with_zero_position(self, tmp_path):
        ...
        assert isinstance(result, MacroSp500TrendStrategy)
    ...
    def test_fit_never_reads_klines_beyond_train_klines(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("research.strategies.macro_sp500_trend.run_backtest", spy)
...
class TestFitNeverTouchesValidateData:
    ...
    isolation -- mirrors `test_macro_real_yield_trend.py`'s identically-
    named class exactly. `TrainableStrategy.fit()`'s own signature never
    receives `validate_klines` at all, so there is no code path by which
    this Trainable *could* read validate data even if it wanted to. This
    test drives it through the real `run_walk_forward` harness (multiple
    ...
    def test_fit_is_only_ever_called_with_each_folds_train_klines(self, tmp_path, monkeypatch):
        ...
        real_fit = MacroSp500TrendTrainable.fit
        ...
        monkeypatch.setattr(MacroSp500TrendTrainable, "fit", spy_fit)

        result = run_walk_forward(
            klines,
            trainable,
            "test-macro-sp500-wf-fit-scope",
            "v1",
            {},
            train_bars=10,
            validate_bars=8,
...
class TestRunWalkForwardIntegrationSynthetic:
    def test_walk_forward_runs_without_error_and_logs(self, tmp_path):
        ...
        trainable = MacroSp500TrendTrainable(
            strategy_id="test-macro-sp500-wf",
            strategy_version="v1",
            fee_bps=Decimal("5"),
            slippage_bps=Decimal("2"),
            macro_observations=macro_observations,
